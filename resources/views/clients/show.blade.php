...
@section('content')
  ...
  @if($client->user_id != 0)
    <form action="/mail/{{$client->user_id}}" id="mailsend_dorkadory" method="POST">
      {{csrf_field()}}
      <input type="hidden" name="id" value="{{$client->usuario}}"></input>                            <button type="submit" form="mailsend_dorkadory" formmethod="post" class="btn btn-sm btn-danger" title="Enviar">
      </form>
    @endif
    <div class="album">
      <div class="container">
